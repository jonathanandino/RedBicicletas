...
app.post('/forgotPassword', function(req, res) {
  Usuario.findOne({email: req.body.email}, function (err, usuario) {
    ...
    usuario.resetPassword(function(err){
      if (err) return next(err);
      console.log('session/forgotPassuordMessage');
    });
...
app.get('/resetPassword/:token', function(red, res, next){
  Token.findOne({ token: req.params.token }, function (err, token) {
    ...
    Usuario.findById(token._userId, function (err, usuario) {
      ...
      res.render('session/resetPassord', {errors: {}, usuario: usuario});
    });
  });
});
...
app.post('/resetPassword', function(req, res){
  if (req.body.password != req.body.confirm_password){
    res.render('session/resetPassuord', {errors: {confirm_password: {message: 'No coincide con el password ingresado'}},
